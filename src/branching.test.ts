...
describe('run function', () => {
  ...
  it('should initiate database branch successfully and set outputs correctly', async () => {
    ...
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'instance_id',
      mockResponseData.instance_id
    )
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'instance_name',
      mockResponseData.instance_name
    )
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'host',
      mockResponseData.connection_info.host
    )
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'port',
      mockResponseData.connection_info.port.toString()
    )
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'user',
      mockResponseData.connection_info.user
    )
    expect(mockedCore.setOutput).toHaveBeenCalledWith(
      'password',
      mockResponseData.connection_info.password
    )
